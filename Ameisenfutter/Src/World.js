//Der Simulator
function World(sizex,sizey,a_count,f_count,f_amount,p_duration,ctx,scale){
    //Der Canvas-2D-Context
    this.ctx = ctx;
    //Die Darstellungsgröße der gesamten Simulation 
    this.scale = scale;
    //Das Objekt zum Aktualisieren der Simulation
    this.updateinterval;
    ...
    this.sizex = sizex;
    this.sizey = sizey;
    //Das Objekt, welches für die regelmäßige aktualisierung sorgt 
    ...
    this.render = function(self){
        //Sichert den Anfangszustand des Canvas-2D-Context
        self.ctx.save();
        //Setzt die Darstellungsgröße
        self.ctx.scale(self.scale,self.scale);
        //Zeichnet alles grün
        self.ctx.fillStyle = "#008000";
        self.ctx.beginPath();
        self.ctx.moveTo(0,0);
        self.ctx.lineTo(0,self.sizey);
        self.ctx.lineTo(self.sizex,self.sizey);
        self.ctx.lineTo(self.sizex,0);
        self.ctx.closePath();
        self.ctx.fill();
        //Zeichnet das Pheromon
        self.ctx.fillStyle = "#0080FF";
        self.ctx.beginPath();
        for(var x=0;x<sizex;x++){
            for(var y=0;y<sizey;y++){
                if(self.pheromon[x][y]>0){
                    self.ctx.moveTo(x,y);
                    self.ctx.lineTo(x+1,y);
                    self.ctx.lineTo(x+1,y+1);
                    self.ctx.lineTo(x,y+1);
                }
            }
        }
        self.ctx.closePath();
        self.ctx.fill();
        //Zeichnet die Futterquellen
        self.ctx.fillStyle = "#FFFF00";
        self.ctx.beginPath();
        for(var i=0;i<self.food.length;i++){
            self.ctx.moveTo(self.food[i].posx,self.food[i].posy);
            self.ctx.lineTo(self.food[i].posx+1,self.food[i].posy);
            self.ctx.lineTo(self.food[i].posx+1,self.food[i].posy+1);
            self.ctx.lineTo(self.food[i].posx,self.food[i].posy+1);
        }
        self.ctx.closePath();
        self.ctx.fill();
        //Zeichnet die Ameisen ohne Futter
        self.ctx.fillStyle = "#FF0000";
        self.ctx.beginPath();
        for(var i=0;i<self.ants.length;i++){
            if(!self.ants[i].food){
                self.ctx.moveTo(self.ants[i].posx,self.ants[i].posy);
                self.ctx.lineTo(self.ants[i].posx+1,self.ants[i].posy);
                self.ctx.lineTo(self.ants[i].posx+1,self.ants[i].posy+1);
                self.ctx.lineTo(self.ants[i].posx,self.ants[i].posy+1);
            }
        }
        self.ctx.closePath();
        self.ctx.fill();
        //Zeichnet die Ameisen mit Futter
        ...
        self.ctx.beginPath();
        self.ctx.moveTo(self.nestx,self.nesty);
        self.ctx.lineTo(self.nestx+1,self.nesty);
        self.ctx.lineTo(self.nestx+1,self.nesty+1);
        self.ctx.lineTo(self.nestx,self.nesty+1);
        self.ctx.closePath();
        self.ctx.fill();
        //Stellt den Anfangszustand des Canvas-2D-Context wieder her
        self.ctx.restore();
    }
    
    //Simuliert ein Simulationsschritt
    this.step = function(self){
        ...
        for(var i=0;i<self.ants.length;i++){
            //Wenn die aktuelle Ameisen Futter trägt
            if(self.ants[i].food){
                ...
                //Wenn die aktuelle Ameise noch nicht auf dem Nestfeld steht
                    ...
                    //Berrechnet die Distanzen der umliegenden Felder der aktuellen
                    ...
                    //Gehe auf das benachbarte Feld der aktuelle Ameise mit der
            ...
            //Wenn die aktuelle Ameise kein Essen trägt
                ...
                if(foodid>=0){
                    //Nimmt die aktuelle Ameise ein Portion Futter
                ...
                //Wenn die aktuelle Ameise auf keiner Futterquelle steht
                    ...
                    //Gibt die Pheromonkonzentrationen der umliegenden Felder an
                    ...
                    //Wenn Pheromon auf den umliegenden Feldern der aktuellen Ameise vorhanden ist
                    else{
                        //Zähle, wieviele der umliegenden Felder der aktuellen Ameise
                        ...
                        //die höchste Pheromonkonzentration haben
                            ...
                            //Gibt die Richtungen, in welchen die Felder mit der höchsten 
                            //Pheromonkonzentration liegen, an
                            var p_dir_max_dir = p_dir.indexesOf(p_dir_max,p_dir_max_count);
                            //Gibt die Distanzen der Felder zum Nest mit der höchsten Pheromonkonzentration
                            //an
                            var p_dir_max_dir_dist = new Array(p_dir_max_count);  
                            //Berechnet die Distanzen der Felder mit der höchsten 
                            //Pheromonkonzentration zum Nest
                            //Geht alle Richtungen der Felder mit den höchsten 
                            //Pheromonkonzentration durch
                            for(var n=0;n<p_dir_max_count;n++){
                                ...
                                switch(p_dir_max_dir[n]){
                                    //Rechts
                                    case 0:
                                        //Berechnet die Distanz von Feld rechts neben der aktuellen Ameise zum Nest 
                                    ...
                                    //Links
                                    case 1:
                                        //Berechnet die Distanz von Feld links neben der aktuellen Ameise zum Nest 
                                        p_dir_max_dir_dist[n] = Math.sqrt(Math.pow(Math.abs(self.nestx-(self.ants[i].posx-1)),2)+Math.pow(Math.abs(self.nesty-(self.ants[i].posy)),2));
                                        break;
                                    //Unten
                                    case 2:
                                        //Berechnet die Distanz von Feld unter der aktuellen Ameise zum Nest 
                                    ...
                                    //Oben
                                    case 3:
                                        //Berechnet die Distanz von Feld über der aktuellen Ameise zum Nest 
                                        p_dir_max_dir_dist[n] = Math.sqrt(Math.pow(Math.abs(self.nestx-(self.ants[i].posx)),2)+Math.pow(Math.abs(self.nesty-(self.ants[i].posy-1)),2));
                                        break;
                                }
                            }
    ...
    this.setupdatespeed = function(value){
        //Stoppt die Simulation 
        this.clear();
        //Setzt die neue Aktualisierungsgeschwindigkeit
        this.updatespeed = value;
        //Startet die Simulation wieder in einer anderen Geschwindigkeit
        this.updateinterval = setInterval(this.update,value,this);
    }
    
    //Stoppt die Simulation 
    this.clear = function(){
        //Falls die Simulation gestartet ist
    ...
    Array.prototype.countOfindexesOf = function(value){
        ...
        for(var i=0;i<this.length;i++){
            //Wenn das aktuelle Element der Array mit Value übereinstimmt, dann
            if(this[i]==value){
